---
title: "IgM promotes genetic exchange of Leishmania: Statistics"
...
This chunk contians two functions that are not common to any packages and are thus defined by me for use in the script
...
# File Paths
BiblioPath1 <- "C:/Users/doehljs/Box/Clump quantification/Dose response_Experiment/"
Output <- "Data Analysis Output/"
FilNam1 <- "Tiago/Data_Summary_In_Vivo_Clump_LLJB_Rearranged.xlsx"
# FilNam1 <- "Tiago/F2_Crosses.xlsx"
FilNam2 <- "Tiago/main tables.xlsx"
# FilNam2 <- "Tiago/main tables F2 Crosses.xlsx"

# Lables
GrpNam <- c( "1BM", "2BM -IgM", "2BM +IgM", "Mouse")
```
...
# Loads all worksheets from xlsx
FigTabDat <- read_all(paste0(BiblioPath1, FilNam2))
```
...
for (aa in 1:length(ClmpTabList)) {
  ...
  for (bb in 1:length(Days)) {
    ...
    tryCatch({fisher.test(ClmpTabList[[aa]][[bb]][["Count"]], simulate.p.value = TRUE); EM <<- 0},
             error = function(e) {EM <<- 1});
    
    if (EM == 0) {
      # Fisher's Exact test for R x C table
      FT <- ClmpTabList[[aa]][[bb]][["Count"]] %>% 
        # { if ("Mice" %in% colnames(.) == TRUE) select(., -Mice) else . } %>% 
        fisher_test(., simulate.p.value = TRUE) # we are excluding the "Mice" data column from the raw data analysis
